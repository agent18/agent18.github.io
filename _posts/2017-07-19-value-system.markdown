...
#### Design rules
<!--
The brain can be imagined as a computer with a ram and storage and some basic rules for it to function, such as say a+b = b+a and so on. Given an infinite ram (computational capability) within an infinitely large storage, it should possible to compute what will happen at a given moment in the future in this world, based upon physics of the universe and an initial condition. But we don't have such infinite ram to make those costly but True computations. So we have some rules within our system to maximize our utility. Such as social proof. Social proof is a behavioural phenominon where we look at others and understand what is the appropriate reponse and our brain awards us for it. If you go to a concert, people feed off of such a crowd. When every one is cheering you can't help but feel the necessity to cheer or chant or do any other stupid thing. The social proof gives everyone a 

We have a brain, which has a finite ram (processing capability) and a finite storage within it. The brain is constantly *attempting* to maximize it's utility. the utility function is a complicated function, with various variables such The primal necessities of an any animal is not to drive in a luxurious car or to have millions of $'s etc., but to have enough food and procreate to pass on your genes [[Citation needed][]].  If you look at the sugar urge(design rule) we have, it was primarily meant to help us get our vitamin-packed foods such as fruits. But unfortunately, we now have non-vitamin-packed products which are sugar packed and satisfy our urge big time. And as it appears, the design rule misfires in this day and age as there are many issues [if we consume shit loads of sugar][sugar].
...
<!--
#### Design rules connecting the true value system

I seem to be computing what I truly want(true value system) by using some rules and as a result, and as a result it might seem that I don't have consistant wants or that my biases are assisting me miscalculate[[Citation needed][]]. This can be observed by the number of studies that call upon this thing called bias. The studies show our behaviour in various situations, along with how they don't make any sense in some cases. The bias based on [scope insensitivity][] for example, tells us a lot about how we are completely incapable about understanding numbers. And so there are many other biases/design rules we seem to have, to compute the real value system. It appears that there is this layer of design rules that are made for connecting the whole real value system based on sensory perceptive inputs.

I seem to be computing what I truly want using some rules. As a result of these rules being not fully representative
-->
...
Now you are left with the option of 
