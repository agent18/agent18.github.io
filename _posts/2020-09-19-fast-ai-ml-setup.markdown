...
layout: post
comments: true
title:  "Setting up the GPU for fastai with Ubuntu and Nvidia"
date: 19-09-2020
categories: notes
tags: ml, dl
...
- If I shut down the gpu by closing the jupyter kernel then all is
  good.
...
1. Based on [Nvidia forum](https://forums.developer.nvidia.com/t/solved-suspend-resuming-and-wakeup-with-nvidia370-28/45282) added the following to grub and updated grub (sudo grub-update)
	...
	Got some warning for this but apprently it doesn't mean anything as
    I have an i5 system ---[Stack](https://askubuntu.com/a/832528/443958)

2.  Didn't change kernel as there was no evidence towards it. People
    changed to 4.17. Ming is currently 4.15

	- [4.17 but it didn't work](https://forums.developer.nvidia.com/t/problem-with-resume-from-suspend-ubuntu-16-04-gt-940mx/51410/63)
	- [Another user](https://forums.developer.nvidia.com/t/problem-with-resume-from-suspend-ubuntu-16-04-gt-940mx/51410/77)
	- [this guy](https://forums.developer.nvidia.com/t/problem-with-resume-from-suspend-ubuntu-16-04-gt-940mx/51410/69)

3. Tried different suspends ([Suspend](https://askubuntu.com/a/1795/443958))
...
4. Tried downgrading.
	...
	It didn't allow me to work with pytorch 1.6 so I wanted to go
    back.
	
5. Tried upgrading back to 430
	...
	Rebooted to black screen of death (tried [this](https://askubuntu.com/a/1096038/443958)) but all seemed
    ok. Added `nomodeset` to grub (In hindsight I could have done:
    `nouveau.modeset=0` I think).
	
	As a last step went to `prime select intel` and it finally allowed
    me to boot with just intel graphics card. Despite having 430
    installed nothing works with it...
	
	Again blank screen and then `noveau.modeset=0` did the trick
	
6. did complete re-install of xserver, unity, lightdm and the nvidia drivers
...
`sudo apt-get install` installs packages and dependences and
recommended.

To remove package we do `sudo apt-get purge` or `sudo apt-get remove
--purge` 
...
This has to be my best recovery so far. It took so much time to prep
...
and read and read, see what other's advice is, know "roughly what the
commands mean". And then plan out the commands and jump into it.

Based mainly on [How to install Nvidia drivers](https://askubuntu.com/questions/61396/how-do-i-install-the-nvidia-drivers), [Graphics issues
in 16.04](https://askubuntu.com/a/760935/443958) and [re-installing xorg and xserver](https://www.computersnyou.com/4945/re-install-xorg-xserver-completely-ubuntu/), [What is gdm3,
kdm, lightdm? How to install and remove them?](https://askubuntu.com/a/829110/443958).
...
Removing dependencies and recommended packages of the purged packages
(if installed with them),
...
Now unity. So in my system ubuntu-desktop was not installed, instead
...
`xorg.conf` file. This I somehow had in the previous installation and
`nvidia-xconfig` didn't bring it back. So I literally copied a back up
and pasted it.
...
System works like in the past. the issue with not being able to sleep
when GPU is full is still pending.

There are [many people](https://forums.developer.nvidia.com/t/problem-with-resume-from-suspend-ubuntu-16-04-gt-940mx/51410/2)who were not able to solve this on their
system. For some users it was concluded that it was a BIOS issue with
their ASUS systems. Read this entire thread [here](https://forums.developer.nvidia.com/t/problem-with-resume-from-suspend-ubuntu-16-04-gt-940mx/51410/2). So if you can't
get it to work. It's ok. It's alright. We're the same and there is no
need to cry.
...
Also python version seesm to have downgraded to python 3.7 (in
fastaicleantest alone)

It seems like a known issue that `conda instal` doesn't work whereas
pip works WTF.
...
Not tested the last part with `graphviz`. While installing
fastaiclean4 I installed graphviz after `gh`. And then had problems
...
```
Tried changing batch size (bs) as suggested by Jeremy but reducing it
to 32 and 16 didn't yield any results. 

A possibility could be to try and free up the space in the GPU used by
...
results. My dual screen wont work and I don't know how to get it to
work. 

While using the system and the GPU it is impossible to do any work
despite the ram being completely free. 

Considering all this, it seems wise to move to paper space. It seems
faster maybe I might have to end up paying money. But that is a choice
i think I can figure out later if necessary.

Sometimes cuda memory becomes full while running the cells fresh... It
also takes a long time. Unsure how to reduce it at the cost of
accuracy.

So I think for now as paperspace is faster and freer and I don't use
emacs anyways... So I don't see the value of trying to get it to work
in my pc. Let's move to paperspace. the error I see that made me do
this is:
...
0. Checked the [kern logs (pastebin link)](https://pastebin.com/B1TNtKPt) but didn't see anything
...
```
1. Based on [Nvidia forum](https://forums.developer.nvidia.com/t/solved-suspend-resuming-and-wakeup-with-nvidia370-28/45282) added the following to grub and updated.
...
3. Blind try: Trying different ([Suspend](https://askubuntu.com/a/1795/443958))s
...
5. Complete remove and re-install of `nvdia-430` as per [here](https://askubuntu.com/a/760935/443958):   
...
6. At this point did a [complete re-install](https://askubuntu.com/a/760935/443958) of `xserver`,`unity`,
